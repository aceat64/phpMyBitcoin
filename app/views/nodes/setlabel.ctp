...
<?php echo $this->Form->create('Node');?>
	<fieldset>
 		<legend><?php echo "Set Label: {$node['Node']['name']}"; ?></legend>
	<?php
		echo $this->Form->input('id',array('value'=>$node['Node']['id']));
		echo $this->Form->hidden('address',array('value'=>$address));
		echo $this->Form->input('address',array('value'=>$address,'disabled'=>'disabled'));
		echo $this->Form->input('label',array('value'=>$label));
	?>
	</fieldset>
